...
repeat {
  ...
  if(length(nextPage) == 0)  break;       #try를 쓴 이유는 사이트마다 nextPage가 0일지 오류가 날지 모르기 때문.
...
#executeScript => 자바스크립트 활용을 위한 코드.
...
repeat{
  ...
  # 자바스크립트를 활용하여 클릭 이벤트 발생시킴.
  # executeScript는 2번째 argument로 list만 올 수 있다.
  Sys.sleep(2)
...
- 물론 RSelenium 라이브러리를 항상 설치하고 실행시키고 cmd창에서도 항상 구동시킨다.